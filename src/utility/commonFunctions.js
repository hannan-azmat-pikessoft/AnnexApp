...
import { changeAppLanguageAction, saveUserAction } from '../redux/actions/userActions';
import reduxStore from '../redux/store';
import Strings from "../translation/language";
import { GOOGLE_MAP_API_KEY, KEY_DEVICE_TOKEN, KEY_IS_GUEST_USER, KEY_USER_DATA, KEY_USER_TOKEN, LANGUAGE } from "./constants";
...
export async function PickSingleDocument(callback, options, maxSize) {
    try {
        ...
        if (res.size > maxSize) {
            flashMessage(Strings.error_file_size + (maxSize / 1000000) + ' mb', 'danger')
            return
        }
        let file = {
            uri: res.uri,
...
export async function searchLocation(query) {
    return new Promise(async (resolve, reject) => {
        ...
        let apiUrl = BASE_URL + "place/textsearch/json?query=" + query + "&key=" + GOOGLE_MAP_API_KEY
...
export async function onLoginSignupSuccess(res, doLogin) {
    storeItem(KEY_USER_DATA, res)
        .then(() => {
            storeItem(KEY_USER_TOKEN, res.token)
                .then(() => {
                    if (doLogin) {
                        loginUser(res)
                    }
...
export function isAudio(file) {
    ...
    if (lastIndex == -1) {
        ...
        var arrayExtensions = ['3gp', 'aa', 'aac', 'aax', 'm4a', 'm4p', 'mp3', 'raw', 'wav', 'wma', 'webm',];
        if (arrayExtensions.lastIndexOf(ext) == -1) {
            return false
        } else {
            return true
