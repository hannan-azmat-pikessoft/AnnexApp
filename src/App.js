...
import { changeInitialLanguage, changeStatusBarColor, getErrorMessage, loginUser, logoutUser } from './utility/commonFunctions';
...
const App = () => {
  ...
  async function checkUpdateRequired() {
    ...
    apiCall(apiData)
      .then(async (versioningRes) => {
        ...
        if (versioningRes.data.versions[Platform.OS].version_force > currentAppVersion) {
  ...
  return (
    <View style={{ flex: 1, }} >
      ...
      <NoInternetModal visible={!isNetwork} onClose={() => onCloseNetworkModal()} />
      <AppUpdateModal
