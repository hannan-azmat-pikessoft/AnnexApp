...
const GuestUserFullScreenLoginModel = ({
    ...
    return (
        <Modal
            ...
            transparent={true}
        >
            <TouchableOpacity activeOpacity={1} style={styles.mainContainer} key="loginFullScreenModel" onPress={() => onClose()} >
                <KeyboardAvoidingView style={{ flex: 1, justifyContent: 'flex-end', }} behavior={Platform.OS == 'ios' ? 'padding' : 'height'} enabled={Platform.OS == 'ios' ? true : true} >
                    <TouchableOpacity activeOpacity={1} style={styles.visibleViewStyle} >
                        <TouchableOpacity style={styles.closeIconContainer} onPress={() => onClose()} >
                            <CommonImage source={Images.IMG_CLOSE} style={{}} viewStyle={{ paddingHorizontal: spacing.PADDING_12, paddingVertical: spacing.MARGIN_12, borderRadius: spacing.MARGIN_50 }} />
                        </TouchableOpacity>
                        <View style={{ flex: 1 }}>
