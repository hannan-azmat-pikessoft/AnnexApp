...
const ServiceTestTypeRow = ({
  ...
  const {serviceInCart, isGuestUser} = useSelector(
    state => ({
  ...
  return (
    <View style={[styles.mainView]}>
      <View style={{flex: 1, marginRight: spacing.MARGIN_12}}>
        ...
        {item.condition ? (
          <RegularText
            style={styles.description}>{`**${item.condition}`}</RegularText>
        ) : null}
        {item.is_fasting_required ? (
          <RegularText
            ...
            ]}>{`${Strings.requires} ${item.required_fasting_duration} ${Strings.hours_of_fasting}`}</RegularText>
        ) : null}
      ...
      {serviceInCart?.service?.id == item.id ? (
        <View style={styles.quantityButtonContainer}>
          <TouchableOpacity
            ...
            <CommonImage
              source={Images.IMG_MINUS_SQUARE}
              style={styles.plusMinusImagestyle}
            />
          </TouchableOpacity>
          <RegularText style={styles.addRemoveButtonText}>
