...
const RenderCarousalText = () => {
    return (
        <View style={styles.carousalMainView} >
            <RegularText style={styles.carousalText} > 100+ {Strings.services} </RegularText>
        </View>
    )
}
...
const TextInputLeftComponent = () => {
    return (
        <View style={{
            ...
            <RegularText style={{ fontSize: textScale(16), marginHorizontal: spacing.MARGIN_16, }} >
                {COUNTRY_CODE}
            </RegularText>
            <View style={{
...
const MobileNumberScreen = ({ onSubmitMobileNo, from }) => {
    ...
    function onSubmit() {
        ...
        if (!validateMobileNo.success) { setMobileNoError(validateMobileNo.msg) } else { setMobileNoError('') }
        if (!validateMobileNo.success) { return }
    ...
    return (
        <ImageBackground style={styles.mainContainer} source={Images.IMG_LOGIN_BG} resizeMode="stretch"  >
            ...
            <View style={styles.signInContainer} >
                ...
                <RegularText style={styles.welcomeText}>{Strings.welcome_back + "! " + Strings.enter_your_number}</RegularText>
                <CommonTextInput
                ...
                {
                    from == KEY_AUTH &&
                    <CommonButton title={Strings.skip}
