...
import { changeStatusBarColor, getImage, navigate } from '../../../../utility/commonFunctions';
import { SCREEN_SERVICE_BOOKING, STATUS_BAR_CONSTANTS } from '../../../../utility/constants';
...
const SelectService = ({ route, }) => {
    ...
    useFocusEffect(
        React.useCallback(() => {
            changeStatusBarColor(colors.appBackgroundColor, STATUS_BAR_CONSTANTS.DARK, serviceInCart.quantity != 0 ? colors.white : colors.appBackgroundColor)
    ...
    return (
        <View style={{ flex: 1, }} >
            <VirtualizedView style={styles.mainContainer} onRefresh={() => { }} refreshing={false} key={"selectService"} >
                <View style={{ flex: 1, paddingHorizontal: spacing.PADDING_16, marginTop: spacing.MARGIN_12 }} >
                    <View>
                        <CommonImage source={getImage(item.cover)} style={styles.imageStyle} viewStyle={styles.imageView} resizeMode={"cover"} />
                        <BackButton style={{ position: 'absolute', top: spacing.HEIGHT_10, left: spacing.HEIGHT_10, ...boxShadow() }} />
                    </View>
            ...
            {serviceInCart.quantity != 0 && !isGuestUser && <CartSummaryRow onPressContinue={onPressContinue} />}
