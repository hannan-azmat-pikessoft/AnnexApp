...
import { changeStatusBarColor, getImage } from '../../../../utility/commonFunctions';
import { ANIMATION_TYPES, EASING_TYPE, SCREEN_NOTIFICATIONS, STATUS_BAR_CONSTANTS } from '../../../../utility/constants';
...
const RenderCarousalImage = ({ item, index }) => {
    return (
        <View style={styles.carousalImageView}  >
            <CommonImage source={getImage(item.image)} style={styles.carousalImage} resizeMode={"cover"} />
        </View>
    )
}
...
const HomeScreen = ({ route, }) => {
    ...
    useFocusEffect(
        React.useCallback(() => {
            changeStatusBarColor(colors.white, STATUS_BAR_CONSTANTS.DARK, colors.white)
        }, [])
    );
    ...
    return (
        <View style={{ flex: 1, }}>
            ...
            <VirtualizedView style={{ flex: 1, backgroundColor: colors.appBackgroundColor }} onRefresh={() => { }} refreshing={false} key={"homeScreen"} >
                {
                    homeBannerFetching ?
                        <SkeletonPlaceholder
                        ...
                        <CarousalComponent
                            ...
                            rowComponent={(item, index) => <RenderCarousalImage item={item} index={index} />}
                ...
                </View>
                {
                    ...
                    <Animatable.View
                        ...
                        <View style={styles.doYouKnowContainer} >
                            <CommonImage source={Images.IMG_HEART_ATTACK} style={styles.doYouKnowContainer_image} />
                            <View style={styles.doYouKnowContainer_textContainer} >
                                <RegularText style={styles.doYouKnowContainer_textContainer_heading} >{homeSuggestionRes[0].question}</RegularText>
                                <RegularText style={styles.doYouKnowContainer_textContainer_description}>{homeSuggestionRes[0].tips}</RegularText>
                            </View>
                        </View>
                    </Animatable.View>
                }
